...
from word_list import spanish_words, get_words_by_length, get_words_up_to_length
...
def password_generator(quantity: int) -> str:
    ...
    while current_length < target_length:
        ...
        if remaining_space >= 3:
            ...
            if available_lengths:
                ...
                available_words = [w for w in spanish_words if len(w) <= remaining_space]
                if available_words:
    ...
    while len(password) < quantity - 2 and len(password_parts) < 6:
        ...
        if remaining_space >= 3:
            available_words = [w for w in spanish_words if 3 <= len(w) <= remaining_space]
            if available_words:
                word = choice(available_words)
...
if __name__ == '__main__':
    ...
    print(f"\n📊 Word list contains {len(spanish_words)} words")

    # Show word length distribution
    from collections import Counter

    length_dist = Counter(len(word) for word in spanish_words)
